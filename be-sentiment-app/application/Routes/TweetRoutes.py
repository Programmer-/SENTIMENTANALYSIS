...
def selectAllTweet():
    ...
    result = Tweet.query.filter_by(
        search_val=hashtag).paginate(int(page), 20, False).items
...
def sentimentCount():
    hashtag = "#"+request.args.get("hashtag")
    sql = db.text('SELECT classification_result AS label, COUNT(`classification_result`) AS result FROM tweet WHERE search_val="' +
                  hashtag+'" GROUP BY(classification_result)')
...
def topTweet():
    hashtag = "#"+request.args.get("hashtag")
    sql = db.text('SELECT id, user_screen_name, text, retweet_count FROM `tweet` WHERE search_val=\'' +
                  hashtag+'\' ORDER BY retweet_count DESC LIMIT 5')
    result = db.engine.execute(sql)
    result_json = jsonify({'result': [dict(row) for row in result]})
    return result_json
...
def dayToDay():
    hashtag = "#"+request.args.get("hashtag")
    sql = db.text('SELECT DATE_FORMAT(created_at, "%Y %M %d") AS dateLabel, classification_result, COUNT(id) AS countSentiment FROM tweet WHERE search_val=\'' +
                  hashtag+'\' GROUP BY DAY(created_at), classification_result LIMIT 30')
    result = db.engine.execute(sql)
    result_json = jsonify({'result': [dict(row) for row in result]})
    return result_json
...
def crawlTweet():
    ...
    authentication.set_access_token(environ.get(
        "ACCESS_TOKEN"), environ.get("ACCESS_SECRET"))
...
#                                            )
        ...
        for tweet in newTweets:
            ...
            local_created_at = created_at.replace(
